...
- Сохранение с `origin: ai`; дозапись в индекс через `data/ai-overlay.json` (retrieve склеивает со статическим `rag-index.json`).
- Итог: тема «структура промпта» даёт Journey, который опирается на существующую статью, в UI метка «сгенерировано AI»; вопрос на `/ask` может найти этот маршрут.
