...
5. Journey из базы — генерация маршрута с опорой на индекс, метка «сгенерировано AI»
...
5. **Journey** — Создать → **Пройти demo** (без ключа) или сгенерировать тему с ключом (маршрут опирается на корпус).
